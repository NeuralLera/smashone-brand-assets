...
5. **Архив — не удаление.** Материал закрытых направлений уезжает в `_archive/<дата>/` с сохранением
   ...
   ⚠️ **Расхождение, которое надо снять решением, а не молчанием:** чистка 02.09.2026 шла по прямому
   указанию «папок `_archive/` не создаём», и снятое (`design-refs`, `heroes/marcus-bell`) удалено, а
   не заархивировано. Одно из двух правил неверно; пока считать действующим указание 02.09, а этот
   пункт — под вопросом.
...
**Цена, замеренная в тот же день:** 24 ссылки дизайн-системы ушли в 404 (12 файлов), Маркус был
определён в фикстурах ТРИЖДЫ и питал шесть превью. Сайт при этом не пострадал — он держит локальные
копии, — и именно поэтому «на сайте всё цело» **не является** проверкой: банк по CDN читает
дизайн-система, а не витрина.
